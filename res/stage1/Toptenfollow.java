...
public class Toptenfollow {
    ...
    public static class TopReducer extends Reducer<Text, Text, Text, Text>{
        ...
        protected void reduce(Text mkey, Iterable<Text> iter, Context context) throws IOException, InterruptedException {
            ...
            for (int k = 0; k < 10; k ++){
                MyCom g = glist.get(k);
                rvalue.set("No." + (k + 1) + " Good: "+ g.getName() + " Followed by " + g.getNum());
    ...
    public static void main(String[] args) throws IOException, ClassNotFoundException, InterruptedException {
        ...
        if (otherArgs.length != 2) {
            System.err.println("Usage: Toptenfollow <in> <out>");
            System.exit(2);
        }
        Job job = Job.getInstance(cfg);
        //设置主方法所在的类
        job.setJarByClass(Toptenfollow.class);
